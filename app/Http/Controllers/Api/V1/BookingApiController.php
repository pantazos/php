...
class BookingApiController extends Controller
{
    ...
    public function store(Request $request): Response
    {
        ...
        // Calculate tax if it's enabled
        $isTaxEnabled = (boolean)Setting::byKey('enable_tax')->first()->value;
        $taxAmount = 0;
        if ($isTaxEnabled) {
            $taxPercentage = (float)Setting::byKey('tax_percentage')->firstOrFail()->value;
            $taxAmount = $service->price * ($taxPercentage / 100);
        }
    ...
    public function leads(Request $request): Response
    {
        ...
        $leads = Booking::byStatus(Status::NEW)->byCategoryArray($providerCategories)->latest()->get();

        return response(BookingResource::collection($leads));
    }
     ...
     * @param $message
     */
    private function sendNotification($token, $title, $message)
    {
        if ($token) {
