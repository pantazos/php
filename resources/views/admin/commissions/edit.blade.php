    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.commissions.update", [$commission->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="key">{{ trans('cruds.commission.fields.key') }}</label>
                <input class="form-control {{ $errors->has('key') ? 'is-invalid' : '' }}" type="text" name="key" id="key" value="{{ old('key', $commission->key) }}" required>
                @if($errors->has('key'))
                    <span class="text-danger">{{ $errors->first('key') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.commission.fields.key_helper') }}</span>
            </div>
            <div class="form-group">
